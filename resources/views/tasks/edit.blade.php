<x-app-layout>
    <div class="max-w-3xl mx-auto p-6">
        <div class="mb-6">
            <h1 class="text-2xl font-bold text-gray-800">Επεξεργασία εργασίας</h1>
            <p class="text-gray-500 mt-1">Ενημέρωσε τα στοιχεία της εργασίας</p>
        </div>
            ...
            <form method="POST" action="{{ route('tasks.update', $task) }}" class="space-y-6">
                ...
                <div>
                    ...
                    <select name="status" class="w-full border border-gray-300 rounded-lg px-3 py-2">
                        <option value="Εκκρεμής" {{ old('status', $task->status) == 'pending' ? 'selected' : '' }}>Εκκρεμής</option>
                        <option value="Σε εξέλιξη" {{ old('status', $task->status) == 'in_progress' ? 'selected' : '' }}>Σε εξέλιξη</option>
                        <option value="Ολοκληρωμένη" {{ old('status', $task->status) == 'completed' ? 'selected' : '' }}>Ολοκληρωμένη</option>
                    </select>
                </div>
